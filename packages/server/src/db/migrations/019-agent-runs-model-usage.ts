 ...
 *
 * SQLite only supports one addColumn per ALTER TABLE statement.
 */
import type { Kysely } from "kysely";

export async function up(db: Kysely<unknown>): Promise<void> {
  await db.schema.alterTable("agent_runs").addColumn("model_usage_json", "text").execute();
  await db.schema.alterTable("agent_runs").addColumn("litellm_cost_usd", "real").execute();
  await db.schema.alterTable("agent_runs").addColumn("sdk_events_json", "text").execute();
}
