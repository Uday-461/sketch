...
export async function runEnrichment(deps: EnrichmentDeps): Promise<EnrichmentResult> {
  ...
  for (let idx = 0; idx < pendingFiles.length; idx++) {
    ...
    try {
      // Mark as processing
      await db.updateTable("indexed_files").set({ embedding_status: "processing" }).where("id", "=", file.id).execute();
  ...
  if (embeddingProvider && chunks.length > 0) {
    try {
      ...
      for (let i = 0; i < storedChunks.length; i++) {
        if (embeddings[i]) {
          await sql`INSERT INTO chunk_embeddings (chunk_id, embedding) VALUES (${storedChunks[i].id}, ${JSON.stringify(embeddings[i])})`.execute(
            db,
          );
        }
      }
...
async function clearFileChunks(db: Kysely<DB>, fileId: string): Promise<void> {
  // Delete embeddings for existing chunks first
